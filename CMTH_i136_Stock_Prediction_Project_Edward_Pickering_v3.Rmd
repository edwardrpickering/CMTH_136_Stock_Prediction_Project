...
```
The histogram suggests that the residuals may not be normal — the left tail seems a little too long.
...
In 2003, Kim used 12 technical indicators to train his Support Vector Machine (SVM) to predict stock price movement.  The www.stockcharts.com website details 55 different Overlays and Technical Indicators that investors can use to make their predictions.    The TTR package in R provides tools to calculate technical indicators from stock data.  The remainder of this section will calculate the relevant indicators for the current data, default values will be used throughout.  
...
```{r}
#create data frame using selected features
filter2.df <- train[c("movement", "clv", "williams_ad.3", "obv", "fastK.2", "trueLow", "trueHigh", "percent_rank", "Close.8", "runmin10.3", "tr")]
...
#10-fold cross validation method used
library("caret")
trainFilter2 <- trainControl(method = "cv", number = 10)
```
...
#K-Nearest Neighbours (knn)
```{r}
fit.knn.filter2 <- train(movement ~. , data = filter2.df, method = "knn",
                 metric = "Accuracy", preProcess = c("range"), trControl = trainFilter2)
```

#Classification & Regression Trees (CART)
```{r}
fit.cart.filter2 <- train(movement ~ . , data = filter2.df, method = "rpart", 
                  metric = "Accuracy", preProcess = c("range"), trControl = trainFilter2)
```

#Naive Bayes (NB)
```{r}
fit.nb.filter2 <- train(movement ~ . , data = filter2.df, method = "nb", 
                  metric = "Accuracy", preProcess = c("range"), trControl = trainFilter2)
```

#SVM with Radial Basis Function (svm)
```{r}
fit.svm.filter2 <- train(movement ~ . , data = filter2.df, method = "svmRadial", 
                  metric = "Accuracy", preProcess = c("range"), trControl = trainFilter2)
```

```{r}
set.seed(5)
results.filter2 <- resamples(list(KNN=fit.knn.filter2, CART=fit.cart.filter2, NB=fit.nb.filter2, SVM=fit.svm.filter2))
summary(results.filter2)
dotplot(results.filter2)
```

```{r}
#select top 5 predictors
filter3 <- cutoff.k(weights, 5)
print(filter3)
```
...
```{r}
#create data frame using selected features
filter3.df <- train[c("movement", "clv", "williams_ad.3", "obv", "fastK.2", "trueLow")]
...
#10-fold cross validation method used
library("caret")
trainFilter3 <- trainControl(method = "cv", number = 10)
```
...
#K-Nearest Neighbours (knn)
```{r}
fit.knn.filter3 <- train(movement ~. , data = filter3.df, method = "knn",
                 metric = "Accuracy", preProcess = c("range"), trControl = trainFilter3)
```

#Classification & Regression Trees (CART)
```{r}
fit.cart.filter3 <- train(movement ~ . , data = filter3.df, method = "rpart", 
                  metric = "Accuracy", preProcess = c("range"), trControl = trainFilter3)
```

#Naive Bayes (NB)
```{r}
fit.nb.filter3 <- train(movement ~ . , data = filter3.df, method = "nb", 
                  metric = "Accuracy", preProcess = c("range"), trControl = trainFilter3)
```

#SVM with Radial Basis Function (svm)
```{r}
fit.svm.filter3 <- train(movement ~ . , data = filter3.df, method = "svmRadial", 
                  metric = "Accuracy", preProcess = c("range"), trControl = trainFilter3)
```

```{r}
set.seed(5)
results.filter3 <- resamples(list(KNN=fit.knn.filter3, CART=fit.cart.filter3, NB=fit.nb.filter3, SVM=fit.svm.filter3))
summary(results.filter3)
dotplot(results.filter3)
```

#Wrapper Feature Selection
```{r}
#Rescursive Feature Elimination
wrapControl <- rfeControl(functions = rfFuncs, method = "cv", number = 10)

results.wrap <- rfe(train[,2:43], train[,1], sizes = c(2:43), rfeControl = wrapControl)

print(results.wrap)
predictors(results.wrap)
plot(results.wrap, type=c("g", "o")
     , main = "Importance of Predictors Variables from Recursive Feature Elimination")
```


```{r}
#create data frame using selected features - importance above 0.02
wrapper1.df <- my_df1[c("movement", "williams_ad.3", "clv", "obv", "fastK.2", "trueLow", "trueHigh", "percent_rank", "high")]
